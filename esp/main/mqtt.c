    ...
    switch (event->event_id) {
        ...
        case MQTT_EVENT_DATA:
            ...
            // Caso contrário, o servidor central pediu para ligar ou desligar o LED
                ...
                int proximo_estado = novo_estado_LED(event->data);
                gpio_set_level(LED, proximo_estado);

                char topicoSaida[50];
                sprintf(topicoSaida, "fse2020/%s/%s/estado", MATRICULA, comodo);
                mqtt_envia_mensagem(
                    topicoSaida,
                    transformar_mensagem_para_JSON(IDENTIFICADOR_ESTADO_SAIDA, mac_address, comodo, proximo_estado)
                );
            }
            break;
...
void mqtt_start() {
    ...
    sprintf(topico_inicializacao, "fse2020/%s/dispositivos/%s", MATRICULA, mac_address);
